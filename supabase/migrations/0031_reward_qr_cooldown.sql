-- Lock the reward QR for the merchant wait window when a card becomes reward_ready.
-- Previously cooldown only started after redeem (stamp lock). Customers expected the
-- "Wait before next reward" setting to keep the QR locked with a countdown.

create or replace function merchant_cooldown_until(p_merchant_id uuid)
returns timestamptz
language plpgsql
stable
security definer
...
declare
  ...
  v_cd_unit text;
begin
  select reward_cooldown_value, reward_cooldown_unit
    into v_cd_value, v_cd_unit
  from merchants
  where id = p_merchant_id;

  if coalesce(v_cd_value, 0) <= 0 then
    return null;
  end if;

  return now() + case coalesce(v_cd_unit, 'days')
...
begin
  ...
  -- After redeem, lock the next stamp card for the configured wait.
  v_until := null;
...
-- Lock any already-unlocked reward QRs that never received a wait timer.
update loyalty_cards lc
set cooldown_until = merchant_cooldown_until(lc.merchant_id)
where lc.status = 'reward_ready'
  and lc.cooldown_until is null
  and merchant_cooldown_until(lc.merchant_id) is not null;

revoke all on function merchant_cooldown_until(uuid) from public, anon;
grant execute on function merchant_cooldown_until(uuid) to authenticated;
