-- Banned guests must not request or receive stamps.
-- offer_stamp already raises on ban; request_stamp / approve_stamp did not.
...
begin
  ...
  select merchant_id, branch_id, banned
  ...
  if coalesce(v_banned, false) then
    raise exception 'You have been banned. Please contact the restaurant directly.';
  end if;
...
begin
  ...
  select banned into v_banned from customers where id = v_customer;
  if coalesce(v_banned, false) then
    update approvals set status = 'rejected', resolved_at = v_now where id = p_approval_id;
