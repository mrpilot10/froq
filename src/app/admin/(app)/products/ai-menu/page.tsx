...
export default async function Page() {
  ...
  const row = data.adoption.byProduct.find((p) => p.product === "menu");
  const mrr = data.revenueByProduct.find((p) => p.product === "menu");
  ...
  return (
    <>
      ...
      <section className="admin-panel">
        <h2 className="admin-panel-title">AI Menu deep analytics</h2>
        <p className="admin-muted">
          Menu views, QR scans, AI chats and translations will deepen as menu_analytics events are aggregated platform-wide. Live adoption and MRR above are production figures from merchant_products.
        </p>
      </section>
    </>
  );
