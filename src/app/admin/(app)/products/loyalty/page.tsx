...
export default async function Page() {
  ...
  const row = data.adoption.byProduct.find((p) => p.product === "loyalty");
  const mrr = data.revenueByProduct.find((p) => p.product === "loyalty");
  ...
  return (
    <>
      ...
      <section className="admin-panel">
        <h2 className="admin-panel-title">Loyalty Stamps deep analytics</h2>
        <p className="admin-muted">
          Campaigns, stamps, rewards and redemption funnels — platform rollup of merchant analytics. Live adoption and MRR above are production figures from merchant_products.
        </p>
      </section>
    </>
  );
