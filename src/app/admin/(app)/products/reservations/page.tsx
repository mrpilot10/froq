...
export default async function Page() {
  ...
  const row = data.adoption.byProduct.find((p) => p.product === "reservation");
  const mrr = data.revenueByProduct.find((p) => p.product === "reservation");
  ...
  return (
    <>
      ...
      <section className="admin-panel">
        <h2 className="admin-panel-title">Reservations deep analytics</h2>
        <p className="admin-muted">
          Bookings, occupancy, no-shows and party-size distributions across the network. Live adoption and MRR above are production figures from merchant_products.
        </p>
      </section>
    </>
  );
