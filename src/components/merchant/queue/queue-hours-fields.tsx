...
export function QueueHoursFields({ value, onChange, compact = false }: QueueHoursFieldsProps) {
        ...
        <div className="merchant-toggle-row queue-hours-recommended">
          <div>
            ...
            <div className="merchant-toggle-desc">
              Start the live queue at opening time and end it at closing time.
            </div>
          </div>
          <button
            type="button"
            role="switch"
            aria-checked={hours.autoSessions}
            className={`merchant-toggle${hours.autoSessions ? " on" : ""}`}
            onClick={() => patch({ autoSessions: !hours.autoSessions })}
          >
...
export function hoursFromProfile(profile: {
  ...
  return {
    ...
    // Either legacy flag counts as “on” until the next save syncs both.
    autoSessions: Boolean(profile.queueAutoStart || profile.queueAutoClose),
  };
}
